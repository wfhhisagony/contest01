...
struct Object {
    ...
    unordered_map<int, int> requests;    // 读取该对象的请求req_id  req_id:start_time

    bool is_deleted;                // 对象是否已被删除
    Object() : id(0), size(0), tag(0), is_deleted(false), disks(3) {
...
public:
    ...
    unordered_map<int, unordered_map<int, ReadRequest>> time_to_requests;  // req_id到链表节点的映射
...
public:
    ...
    unordered_map<int, ReadRequest> overdue_requests;
    ...
    // 删除指定req_id的请求，O(1)
    unordered_map<int, ReadRequest>::iterator eraseByTime(int req_id, int currenttime) {
        auto it = time_to_requests[currenttime].find(req_id);
        if (it != time_to_requests[currenttime].end()) {
            return time_to_requests[currenttime].erase(it);
        }
        return time_to_requests[currenttime].end();
    }

    bool contains(int req_id) const {
        for (auto& it : time_to_requests) {
            if (it.second.find(req_id) != it.second.end()) {
                return true;
            }
        }
        return false;
    ...
    // 通过req_id访问节点
    ReadRequest* getByTime(int req_id, int currenttime) {
        auto it = time_to_requests[currenttime].find(req_id);
        if (it == time_to_requests[currenttime].end()) {
            return nullptr;
        }
    ...
    void push(int req_id, int obj_id, int currenttime) {
        time_to_requests[currenttime].emplace(req_id, ReadRequest(req_id, obj_id, currenttime));
        current_size++;
    }

    // 弹出队头请求(删除过期的请求)，O(1)
    void pop_oldestByTime(int currenttime, map<int, int>& target_blocks) {
        int oldest_time = currenttime - 105;
        if (oldest_time < 1) return;
        auto it = time_to_requests.find(oldest_time);
        if (it != time_to_requests.end()) {
            current_size -= it->second.size();
            for (auto& it2 : it->second) {
                ReadRequest& req = it2.second;
                Object& obj = Objects[req.obj_id];
                for (int i = 0; i < obj.size; ++i) {
                    if (req.blocks_done[i] == false) {
                        auto it = target_blocks.find(obj.units[i]);
            ...
            time_to_requests.erase(it);
        }
        else return;
        /*while (remain_delete_size > 0) {
            oldest_time++;
            if (oldest_time == currenttime) break;
            it = time_to_requests.find(oldest_time);
            if (it != time_to_requests.end()) {
                if (remain_delete_size >= it->second.size()) {
                    remain_delete_size -= it->second.size();
                    for (auto& it2 : it->second) {
                        current_overdue_requests.push_back(it2.second);
                    }
                    time_to_requests.erase(it);
                }
                else {
                    remain_delete_size = 0;
                }
            }
            else break;
        }*/
    ...
    // 弹出队头请求(删除过期的请求)，O(1)
    void pop_oldestByTime(int currenttime, vector<map<int, int>>& target_blocks, unordered_map<int, int>& virtualIndex_virtualGroup_map) {
        int oldest_time = currenttime - 105;
        if (oldest_time < 1) return;
        int remain_delete_size = max_que_size;
        auto it = time_to_requests.find(oldest_time);
        if (it != time_to_requests.end()) {
            remain_delete_size -= it->second.size();
            for (auto& it2 : it->second) {
                ...
                auto it3 = virtualIndex_virtualGroup_map.find(vdisk_idx);
                if (it3 != virtualIndex_virtualGroup_map.end()) {
                    int group_idx = it3->second;
                    // 移除所有访问块 (使用相对位置)
                    for (int i = 0; i < obj.size; ++i) {
                        if (req.blocks_done[i] == false) {
                            auto it_tmp = target_blocks[group_idx].find(obj.units[i]);
                            if (it_tmp == target_blocks[group_idx].end()) continue;
                            if (it_tmp->second == 1) {
                                target_blocks[group_idx].erase(it_tmp);
                            }
                            else {
                                it_tmp->second--;
                            }
...
public:
    ...
    virtual void add_request(int req_id, int obj_id, int currenttime) = 0;   // 读取
    virtual void pop_oldest(int currenttime) = 0;   // 读取
    virtual void delete_object(Object& obj) = 0;  // 删除

    // virtual pair<int, int> get_sort_info(int disk_index) = 0; // 获取磁盘排序信息
    virtual void relocate_next_read_distance(int disk_index) = 0;   // 重新获取head.next_read_distance
    virtual void diskHead_sortByConcentration(vector<int>& disk_index_list) = 0;    // 排序磁盘下标


    void process_readRequests(int currenttime, vector<int>& total_completed_requests) {
        ...
        while (true) {
            ...
            for (int disk_index : sorted_disk_index_list) {
                ...
                relocate_next_read_distance(disk_index);
        ...
        // 检查已完成的请求
        int end_time = max(currenttime - 105, 1);
        for (int ctime = currenttime; ctime > end_time; ctime--) {
            unordered_map<int, ReadRequest>& it = active_requests.time_to_requests[ctime];
            unordered_map<int, ReadRequest>::iterator it2 = it.begin();
            while (it2 != it.end()) {
                ReadRequest& req = it2->second;
                ...
                if (finish_cnt == req.block_num) {
                    total_completed_requests.push_back(req.req_id);
                    obj.requests.erase(req.req_id);
                    it2 = active_requests.eraseByTime(req.req_id, ctime);
                }
                else {
                    ++it2;
                }
...
public:
    ...
    */
    bool add_object(Object& obj) {
        ...
        obj_set.emplace(obj.id);
        return true;
    }


    // 为读取请求添加目标块
    void add_request(int req_id, int obj_id, int currenttime) override {
        active_requests.push(req_id, obj_id, currenttime);
        Object& obj = Objects[obj_id];
        for (int i = 0; i < obj.size; ++i) {
    ...
    void pop_oldest(int currenttime) override {
        active_requests.pop_oldestByTime(currenttime, target_blocks);
    }
    ...
    void relocate_next_read_distance(int disk_index) {
        DiskHead& head = disk_heads[disk_index];
        if (target_blocks.empty()) {
            head.next_read_distance = 0;
            head.concentration = 1; // 不用调度
            return;
        }
    ...
    void minMaxReadStrategy(int disk_index, vector<char>& actions) override {  // 一个时间片中，每个disk只会调用一次该方法
        ...
        if (head.remaining_tokens > 0 && head.pos != next_uint_index) {
            ...
            if (steps >= G) { // Jump
                ...
                head.last_action = 'P';
                actions.insert(actions.end(), steps, 'p');
                if (head.remaining_tokens < 64) {   // token用完，无法进行其他操作，返回
                    return;
                }
            }
        }
...
public:
    ...
    */
    ...
    void add_request(int req_id, int obj_id, int currenttime) override {
        ...
        int group_idx = virtualIndex_virtualGroup_map[vdisk_idx];
        // 添加所有块到目标集合 (使用相对位置)
        for (int i = 0; i < obj.size; ++i) {
    ...
    void pop_oldest(int currenttime) override {
        active_requests.pop_oldestByTime(currenttime, target_blocks, virtualIndex_virtualGroup_map);
    }
    ...
    void relocate_next_read_distance(int disk_index) {
        ...
        if (related_vdisk_index == 3 || head.pos > all_target_units_copy[all_target_units_copy.size() - 1]) {  // head.pos在最后不使用的磁盘块上,这时其对应的下一个虚拟盘应为磁盘最开头的虚拟盘
            ...
            if (index & 1) { // 奇，说明是max
                ...
                int start_unit = min_unit_index / chunk_size * chunk_size;
                head.next_read_distance = (min_unit_index - head.pos + V) % V;
                window_size = min((head.remaining_tokens - 150) / 16, max_unit_index - min_unit_index + 1);
                target_count = 0;
                auto it_tmp = target_blocks[gourp_idx].begin();
                for (int j = min_unit_index; j <= min_unit_index + window_size; j++) {
                    if (j == it_tmp->first + start_unit) {
    ...
    void continuouslyReadStrategy(DiskHead& head, int disk_index, vector<char>& actions) {
        ...
        int flag = (target_blocks[group_id].rbegin()->first + start_unit + 1) / chunk_size;
...
public:
    ...
    void initialize_disk_groups() {
        ...
        for (int i = 0; i < three_group_count; ++i) {
            ...
            disk_groups.push_back(new DiskGroupThree(V, G, i * 3 + 1, 30, group_disks));
        }
        ...
        if (remaining_disks == 4) {
            ...
            disk_groups.push_back(new DiskGroupFour(V, G, three_group_count * 3 + 1, 70, group4_disks));
        }
        else // (remaining_disks == 5) 
        {
            // 创建5盘组
            vector<int> group5_disks(all_disks.begin() + three_group_count * 3, all_disks.end());
            disk_groups.push_back(new DiskGroupFive(V, G, three_group_count * 3 + 1, 70, group5_disks));
        }
    }
    ...
    void handle_read_events(int current_time) {
        ...
        vector<int> add_req_cnt(disk_groups_size, 0);
        int total_req_cnt = 0;
        for (int i = 0; i < disk_groups_size; i++) {
            total_req_cnt += disk_groups[i]->active_requests.max_que_size;
        }
        for (int i = 0; i < n_read; ++i) {
            ...
            // 关联到对象
            Objects[obj_id].requests.emplace(req_id, current_time);
            if (total_req_cnt == 0) continue;
            // 关联到磁盘组
            for (int j = 0; j < disk_groups_size; ++j) {
                if (disk_groups[j]->obj_set.count(obj_id) && add_req_cnt[j] < disk_groups[j]->active_requests.max_que_size) {
                    total_req_cnt--;
                    add_req_cnt[j]++;
                    disk_groups[j]->add_request(req_id, obj_id, current_time);
                    break;
                }
            }
        }
        // 删除过期请求
        for (int i = 0; i < disk_groups_size; ++i) {
            DiskGroup* disk_group = disk_groups[i];
            disk_group->pop_oldest(current_time);
        }


        // 输出无请求取消（题目保证输入有效）
        vector<int> total_completed_reqs;
        total_completed_reqs.reserve(G / 16 * 10);
        for (int i = 0; i < disk_groups_size; ++i) {
            disk_groups[i]->process_readRequests(current_time, total_completed_reqs);
        }
    ...
    void handle_delete_events(int currenttime) {
        int n_delete;
        scanf("%d", &n_delete);
        ...
        for (int i = 0; i < del_ids.size(); i++) {
            ...
            Object& obj = Objects[obj_id];
            for (auto& it : obj.requests) {
                printf("%d\n", it.first);
                disk_groups[objInDiskGroupIndex[i]]->active_requests.eraseByTime(it.first, it.second);
            }
