...
struct Replica {
    int disk_id;                // 副本所在硬盘编号（1 ≤ disk_id ≤ N）
    int start_block;            // 副本起始存储单元编号
    int length;                 // 副本占用的连续存储单元数（等于对象大小）
    bool is_valid;              // 副本是否有效（未被删除）
};

struct Object {
    int id;                     // 对象唯一标识
    int size;                   // 对象块数量（1 ≤ size ≤ 5）
    int tag;                    // 对象标签（1 ≤ tag ≤ M）
    Replica replicas[REP_NUM + 1];        // 三个副本的详细信息
    int* unit[REP_NUM + 1]; // 记录该对象在各个副本中的块位置

    // vector<vector<int>> units; // 每个副本的块位置（units[replica_index][block_index])
...
typedef struct DiskHead_ {
    int pos;            // 当前磁头位置
    char last_action;   // 上一次动作类型（'J', 'R', 'P'）
    int last_token;     // 上一次动作消耗的令牌
} DiskHead;
DiskHead heads[MAX_DISK_NUM]; // 每块硬盘的磁头状态
...
class Disk{
    ...
    int id;                     // 硬盘编号
    bitset<MAX_DISK_SIZE> bitmap; // 存储单元占用状态（位图）
    int free_count;   // 当前空闲单元数（动态更新，避免遍历位图）

    // DiskHead head_state;         // 磁头当前状态（位置、动作类型等），就是下面的属性
    int pos;            // 当前磁头位置
    int disk_objectChunk[disk_objectChunk_size];    // 磁盘块，里面存对象的块号
    char last_action;   // 上一次动作类型（'J', 'R', 'P'）
    int last_token_index;     // 上一次读消耗的令牌下标
    // unordered_set<int> disk_objectId_set;    // 快速查找存储了哪些对象，用于读取操作
    int get_read_token(DiskHead* head) {
        if (this->last_action != 'R') last_token_index = 0;
        else if( last_token_index < 7) last_token_index++;
        return read_token_consum[last_token_index];
    }

    public:
    // 分配指定数量的空闲单元（不要求连续）
    bool allocate_blocks(int size, std::vector<int>& allocated_units);

    // 释放指定存储单元
    void free_blocks(const std::vector<int>& units);

    // 获取当前空闲单元数
    int get_free_count() const { return free_count; }

    // 标签优化分配（预留高频标签的空闲单元）
    bool allocate_for_tag(int tag, int size, std::vector<int>& units);
}


class TagManager {
private:
    unordered_map<int, int> tag_preferred_disk; // 标签到推荐硬盘的映射
    unordered_map<int, int> tag_freq;          // 标签访问频率统计

public:
    // 全局预处理阶段加载标签统计数据
    void load_statistics(const vector<int>& fre_read);

    // 为指定标签选择最优硬盘（优先高频访问标签的预留区域）
    int select_disk_for_tag(int tag, int size);
};

class StorageSystem {
    private:
        std::vector<Disk> disks;            // 所有硬盘（disks[0]未使用，disk_id从1开始）
        std::unordered_map<int, Object> objects; // 所有对象（key为对象ID）
        TagManager tag_manager;             // 标签优化控制器
    
    public:
        // 初始化硬盘和标签管理器
        void initialize(int disk_count, int disk_size);
    
        // 处理写入请求（核心逻辑）
        bool write_object(int obj_id, int size, int tag);
    
        // 处理删除请求
        void delete_object(int obj_id);
    
        // 全局预处理接口
        void preprocess(const std::vector<int>& fre_read);
};
