...
void handle_read_requests(int current_time) {
    ...
    //int front_process_request_num = N_PROCESS_REQUEST / 8 * 7;
    //int end_process_request_num = min(N_PROCESS_REQUEST / 8, int(activeRequests.queue.size() - front_process_request_num));
    int end_process_request_num = N_PROCESS_REQUEST;
    vector<int> to_delete_reqId;
    ...
    //        if (obj.disks[replica_order[j]] & 1) {  // 奇数
    //            int tmp = replica_order[i];
    //            replica_order[i] = replica_order[j];
    //            replica_order[j] = tmp;
    //            i++;
    //        }
    //    }
    ...
    //            
    //        break;
    //    case 1: // 1个奇2个偶, 奇的先找，偶的路近的先找，另一个按兵不动(或者其他策略)
    //        break;
    //    case 2: // 2个奇1个偶，偶的先找，奇的路近的先找，另一个按兵不动
    //        break;
    //    default:
    //        break;
    //    }
